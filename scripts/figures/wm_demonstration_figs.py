...
def fig_params_breakdown(summary: dict) -> Path:
    ...
    for i, (bar, v, pct) in enumerate(zip(bars, values, pcts)):
        label = f"{v:,.0f}  ({pct:.1f}%)"
        x_end = bar.get_width()
        ax.text(x_end * 1.06, i, label, va="center", ha="left",
    ...
    ax.xaxis.grid(True, which="both", color=RULE, linewidth=0.5, linestyle="--", alpha=0.6)
    ax.set_axisbelow(True)

    ax.set_title(
        "Where the 1.034 billion parameters live",
        fontsize=14, color=DARK, pad=10, fontweight="bold",
    )
    fig.text(
        0.5, -0.04,
        ...
        ha="center", va="top", fontsize=10, color=RULE,
        wrap=True,
    )
...
def fig_data_coverage(summary: dict) -> Path:
    ...
    train_limit = summary["train_discovery_limit"]          # 4000
    ...
    def two_bar(ax, used, total, label_used, label_rest, title, unit=""):
        ...
        bars = ax.barh([1, 0], vals, color=colors, height=0.5, edgecolor="none")
        ax.set_yticks([0, 1])
        ax.set_yticklabels(["rest", label_used], fontsize=12, color=DARK)
        ax.set_xlim(0, total * 1.35)
        pct = used / total * 100
        ax.text(used + total * 0.02, 1, f"{used:,} / {total:,}  ({pct:.1f}%)",
                va="center", ha="left", fontsize=11, color=DARK)
        ax.text(total - used + total * 0.02, 0, f"{total-used:,}",
                va="center", ha="left", fontsize=11, color=RULE)
    ...
    two_bar(ax_a, trained_shots, total_shots, "trained", "not used",
            "(a) Shots trained / available")
    two_bar(ax_b, tokens_used, tokens_total, "used tokens", "not used",
...
def fig_skill_by_modality(summary: dict) -> Path:
    ...
    for i, (bar, s) in enumerate(zip(bars, skills)):
        ...
        ax.text(s + x_off, i, f"{s:+.4f}", va="center", ha=ha,
                fontsize=9.5, color=DARK)

    # Overall reference line
    ax.axvline(overall_skill, color=DARK, linewidth=1.5, linestyle="--", alpha=0.85)
    ax.text(overall_skill + 0.002, len(labels) - 0.3,
            f"overall {overall_skill:+.4f}", fontsize=9.5, color=DARK, va="top")
    ...
    ax.set_xlabel("Skill = 1 − model_error / persistence_error", fontsize=11, color=DARK)
    ax.tick_params(colors=RULE, labelcolor=DARK)
    ...
    ax.legend(handles=[win_patch, loss_patch], fontsize=10, frameon=False,
              loc="lower right")
    ...
    fig.text(
        ...
        ha="center", va="top", fontsize=9.5, color=RULE,
        wrap=True,
    )
    fig.tight_layout()
...
def fig_signal_traces(bundle: np.lib.npyio.NpzFile) -> tuple[Path, list[dict]]:
    ...
    channels = [
        dict(modality="gas_injection", ch=0,
             note="gas injection (worst-case modality, skill −0.39)"),
        dict(modality="pf_active",    ch=2,
             note="poloidal field coil (electromagnetic, 34 unique tokens)"),
        dict(modality="xsx",          ch=0,
             note="soft x-ray line (plasma thermal emission)"),
    ]
    ...
    for ax, ch_info in zip(axes, channels):
        ...
        ax.plot(steps, t_masked, color=ACCENT_TRUTH, lw=1.5, zorder=3,
                label="truth", solid_capstyle="round")
        ax.plot(steps, p_masked, color=ACCENT_PRED,  lw=1.5, zorder=4,
                linestyle="--", label="model pred")
        ax.plot(steps, r_masked, color=ACCENT_PERS,  lw=1.0, zorder=2,
                linestyle=":", label="persistence")

        ax.set_ylabel("token id", fontsize=10, color=DARK)
        ax.set_title(f"{mod}  ch {c}", fontsize=12, color=DARK, pad=4, fontweight="bold")
        ax.tick_params(colors=RULE, labelcolor=DARK)
        for spine in ["top", "right"]:
            ax.spines[spine].set_visible(False)
        for spine in ["left", "bottom"]:
    ...
    axes[0].text(CONTEXT / 2, axes[0].get_ylim()[1],
                 "context", ha="center", va="bottom",
                 fontsize=9, color=RULE, style="italic")

    fig.suptitle(
        f"Predicted vs actual token trajectories (held-out shot {SHOT})",
        fontsize=14, color=DARK, y=1.01, fontweight="bold",
    )
    fig.text(
        0.5, -0.02,
...
def fig_camera_gt_gif(cam: np.lib.npyio.NpzFile) -> Path:
    ...
    def _add_title_strip(frame_np: np.ndarray, title: str, step: int,
                         n_frames: int) -> Image.Image:
        ...
        draw.text((4, 2), f"{title}  [frame {step+1}/{n_frames}]",
                  fill=(220, 220, 220))
        return canvas
...
def fig_camera_pred_vs_truth_gif(bundle: np.lib.npyio.NpzFile) -> Path:
    ...
    for step in range(n_steps):
        ...
        # Panel 0: truth
        axes[0].imshow(t_grid, vmin=vmin, vmax=vmax,
                       cmap=cmap_signal, interpolation="nearest")
        axes[0].set_title("truth", fontsize=11, color=DARK)

        # Panel 1: model prediction
        axes[1].imshow(p_grid, vmin=vmin, vmax=vmax,
                       cmap=cmap_signal, interpolation="nearest")
        ...
        axes[2].imshow(match, vmin=0, vmax=1,
                       cmap=cmap_match, interpolation="nearest")
        ...
        if valid_mask.any():
            ...
            axes[2].text(0.5, -0.12, f"{frac:.0%} match",
