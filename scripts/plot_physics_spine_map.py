...
def main() -> None:
    ...
    ax.text(
        0.5,
        0.972,
        "GS forward / physics-spine — electromagnetic stack map",
        ha="center",
        fontsize=13.5,
        fontweight="bold",
    )
    ...
    # ---- Tier labels ----
    for ty, lab in (
        (0.80, "CONSUMERS / SOLVE"),
        (
            0.55,
            "COUPLING ASSEMBLY  (gs/operator · latent/{boundary_disc,temporal_operator})",
        ),
    ...
    box(
        ...
        0.10,
        "gs_solve.py",
        "interior force-balance\nsolve (2659 LOC)",
        LIVE,
    )
    box(ax, 0.23, 0.83, 0.14, 0.10, "engine.py", "learned operator\nloss terms", LIVE)
    box(
        ax, 0.39, 0.83, 0.19, 0.10, "spine_bench/runner", "frozen-metric\nharness", LIVE
    )
    box(
        ax,
        0.60,
    ...
    box(
        ...
        0.135,
        "build_operator",
        "G_pf / G_passive / G_plasma\npoint-observer, finite-area src",
        DUP,
    )
    box(
        ax,
        ...
        "a_sens / g_grid\npoint-observer  (delegated to\nby build_passive_circuit_system)",
        DUP,
    )
    box(
        ax,
    ...
    box(
        ...
        0.075,
        "description_reader",
        "declared map — every product caller",
        LIVE,
        alpha=0.10,
    )
    box(
        ...
        "artifact read — no product caller",
        UNWIRED,
        alpha=0.12,
    )
    ax.text(
