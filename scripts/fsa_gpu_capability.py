...
Writes a stamp to imas_ambix/spine_bench/results/fsa-gpu-capability-<commit>-<host>.yaml.
"""
...
def main() -> int:
    ...
    for n in batch_sizes:
        ...
        with jax.default_device(jax.devices("cpu")[0]):
            ...
            diff = float(
                np.max(np.abs(np.asarray(out["inv_r2"]) - np.asarray(out_cpu["inv_r2"])))
            )
    ...
    print(
        ...
        f"max|Δ| vs cpu={max_diff_vs_cpu:.2e}  →  "
        f"{'GPU CAPABILITY DEMONSTRATED' if ok else ('CPU-only (no GPU device)' if not on_gpu else 'PARITY FAIL')}"
    )
    print(f"stamp: {path}")
    return 0
