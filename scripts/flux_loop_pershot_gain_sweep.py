...
def ef_stats(shot):
    try:
        ...
        out = {}
        for names, tag in (
            (("error_field_a", "error_field_02"), "ef_a"),
...
def per_shot(shot):
    ...
    for name, i in IDX.items():
        ...
        if good.sum() < 100 or np.ptp(x[good]) < 1e-12:
            continue
...
def main():
    ids = json.loads(
        open("/work/projects/imas_gpu/mast/manifests/level1-all.json").read()
    )["shot_ids"]
    ...
    for i, s in enumerate(shots):
        ...
        if i % 40 == 0:
            print(
                f"{i}/{len(shots)} done, {len(out)} valid", file=sys.stderr, flush=True
            )
    json.dump(
        out,
        open(
            "imas_ambix/latent/artifacts/patch_gate/flux_loop_pershot_gains.json", "w"
        ),
    )
