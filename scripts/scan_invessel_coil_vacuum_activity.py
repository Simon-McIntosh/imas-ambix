...
def scan(shot: int) -> dict | None:
    ...
    rec = {
        "shot": shot,
        "peak_ip_ka": peak,
        "ef2": _peak(g, "amc", "error_field_02"),
        "ef5": _peak(g, "amc", "error_field_05"),
        "vacuum": peak < VAC_PEAK_KA,
    }
    if peak < VAC_PEAK_KA:
        elm = {}
...
def main() -> int:
    ...
    ap.add_argument(
        "--manifest", type=str, default=str(MANIFEST_DIR / "level1-all.json")
    )
    ap.add_argument(
        "--drive-floor",
        type=float,
    ...
    for i, s in enumerate(shots):
        ...
        if i % 500 == 0:
            ...
            logger.info(
                "%d/%d scanned, %d valid, %d vacuum", i, len(shots), len(recs), n_vac
            )
    ...
    strong = sorted(
        (r for r in vac_ivc if topval(r) > args.drive_floor), key=topval, reverse=True
    )
    ...
    ledger = {
        ...
        "error_field_05_median": float(
            np.median([r["ef5"] for r in recs if r["ef5"] is not None])
        ),
        "candidates": [
            {
                "shot": r["shot"],
                "era": _era(r["shot"]),
                "peak_ip_ka": round(r["peak_ip_ka"], 2),
                "ef5": r["ef5"],
    ...
    logger.info(
        "wrote %s: %d strong candidates %s",
        out,
        ledger["n_strong"],
        ledger["era_dist_strong"],
    )
    return 0
