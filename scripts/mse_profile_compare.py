    ...
    *,
    ...
    for k in order:
        ...
        f_basin = _fit(
            ...
            n_f_=1,
            nonneg_=False,
            warm=warm_basin if warm_basin is not None else seed,
            centroid=centroid,
        )
        ...
        slices.append(
            {"p": p, "centroid": centroid, "basin": f_basin.jphi_flat if conf else seed}
        )
    if len(slices) < 2:
        return {"shot": shot, "slices": []}
    ...
    for s in slices:
        ...
        if (
            fu.scored
            and fu.psi is not None
            and fu.coeffs is not None
            and _confined(_axis(fu)[0])
        ):
            geos.append(
                flux_surface_geometry(
                    fu.psi,
                    grid,
                    coeffs=np.asarray(fu.coeffs, float),
                    ip_amperes=abs(float(fu.ip_amperes)),
                    n_p=n_p,
                    n_f=n_f,
                    nonneg=nonneg,
                    b_phi0=bt0,
    ...
    for j in range(len(slices) - 1):
        ...
        if out is not None:
            preds[j + 1] = out
            budgets[j] = out["budget"] | {
                "t0": slices[j]["p"].time_s,
    ...
    for j, s in enumerate(slices):
        ...
        if fc.scored and fc.psi is not None and fc.coeffs is not None and _confined(cr):
            geo = flux_surface_geometry(
                fc.psi,
                grid,
                coeffs=np.asarray(fc.coeffs, float),
                ip_amperes=abs(float(fc.ip_amperes)),
                n_p=n_p,
                n_f=n_f,
                nonneg=nonneg,
                b_phi0=bt0,
            ...
            if geo is not None:
                jtor = np.asarray(
                    predicted_current(
                        geo, geo.psi_face, np.zeros_like(geo.psi_face), eta
                    )["j_tor"],
                    float,
                )
...
def ours_pitch(row, rpos) -> np.ndarray:
    ...
    from imas_ambix.statespace.mse_eval import (
        pitch_from_current_profile,  # noqa: PLC0415
    )
    ...
    return pitch_from_current_profile(
        np.asarray(row["jtor"], float),
        rho_m,
        np.asarray(rpos, float),
        R0_M,
        row["bt0"],
        kind="j",
    )
...
def run_enkf(shots, manifest) -> dict:
    ...
    for sid in shots:
        ...
        if e and e.get("partition") == "held_out":
            grid[int(sid)] = {
                "t": np.asarray(e["beam_on_slice_times"], float),
                "rpos": np.asarray(e["active_channel_rpos"], float),
            }
    _preds, results = enkf.predict_shots(
        shots, cfg, arm="analysis", return_results=True, manifest_grid=grid
    )
    out = {}
    for sid, res in results.items():
        pm = np.nanmean(res.pitch_samples, axis=2)  # (K,C)
        out[int(sid)] = {
            "t": np.asarray(res.slice_t, float),
            "pitch": pm,
            "j": None if res.j_analysis is None else np.asarray(res.j_analysis, float),
            "rho": None
            if res.rho_analysis is None
            else np.asarray(res.rho_analysis, float),
        }
    return out
...
def pick_times(entry, ours) -> list[tuple[str, float, int]]:
    ...
    return [
        ("ramp-up", float(t_beam[k_ru]), k_ru),
...
def fig_j_compare(data, path):
    ...
    fig, axes = plt.subplots(
        len(shots), ncol, figsize=(4.0 * ncol, 3.3 * len(shots)), squeeze=False
    )
    for i, sid in enumerate(shots):
        ...
        for c, (lab, t, k) in enumerate(d["times"]):
            ...
            ax.set_xlabel("ρ (norm. minor radius)")
            ax.set_xlim(0, 1)
            if c == 0:
    ...
    fig.suptitle(
        "Toroidal current-density profile: our reanalysis engine vs EnKF/TORAX",
        fontsize=10,
    )
    ...
    for i, sid in enumerate(shots):
        ...
        for c, (lab, t, k) in enumerate(d["times"]):
            ...
            if tr is not None and k < np.asarray(tr.pitch).shape[0]:
                from imas_ambix.statespace.mse_split import (
                    pitch_point_gate,  # noqa: PLC0415
                )

                g = pitch_point_gate(tr.pitch[k : k + 1], tr.pitch_error[k : k + 1])[0]
                pe = np.where(
                    np.isfinite(tr.pitch_error[k]) & (tr.pitch_error[k] > 0),
                    tr.pitch_error[k],
                    np.nan,
                )
                ax.errorbar(
            ...
            if row is not None:
                ax.plot(
                    rpos, ours_pitch(row, rpos), "-", color="#268", lw=1.8, label="ours"
                )
            ek = d["enkf"].get(int(sid))
            ...
            if i == 0 and c == 0:
                ax.legend(fontsize=7)
    fig.suptitle(
        "MSE pitch: our engine and EnKF vs the held-out MSE truth", fontsize=10
    )
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    fig.savefig(path, dpi=130)
    plt.close(fig)
...
def fig_ejima(data, path):
    shots = list(data)
    fig, axes = plt.subplots(
        1, len(shots), figsize=(4.6 * len(shots), 3.6), squeeze=False
    )
    for i, sid in enumerate(shots):
        d = data[sid]
        r0 = d["ours"]["r0"]
        ...
        for b in buds:
            ...
            ramp.append(is_ramp)
            ce.append(
                abs(b["d_psi_axis"]) / (MU0 * r0 * abs(dip))
                if is_ramp and abs(dip) > 0
                else np.nan
            )
        tm = np.array(tm)
        ce = np.array(ce)
        ...
        ax = axes[0][i]
        ax.plot(
            tm, vres, "o-", color="#268", lw=1.4, ms=3, label="resistive V$_{axis}$ [V]"
        )
        ax2 = ax.twinx()
        ax2.plot(
            tm[ramp],
            ...
            lw=1.4,
            ms=4,
            label="Ejima C$_E$ (ramp)",
        )
        ax.set_title(f"shot {sid} — flux consumption", fontsize=9)
        ax.set_xlabel("time [s]")
        ax.set_ylabel("resistive axis loop voltage [V]", color="#268")
        ax2.set_ylabel("incremental Ejima C$_E$", color="#c66")
        lines = ax.get_lines() + ax2.get_lines()
        ax.legend(lines, [ln.get_label() for ln in lines], fontsize=7, loc="best")
    fig.suptitle(
        "Ejima resistive flux-consumption backed out of our engine's flux ledger "
        "(EnKF/TORAX C$_E$ not exposed by the baseline)",
        fontsize=9,
    )
...
def main() -> int:
    ...
    for sid in shots:
        ...
        logger.info(
            "shot %d: %d confined slices, times=%s",
            sid,
            sum(1 for r in ours["rows"] if r["confined"]),
            times,
        )
