"""The lane publisher must leave a readable failure receipt in the router log."""

from __future__ import annotations

import asyncio
import contextlib
import gzip
import json
import logging
import math

import pytest
from aiohttp import web

from imas_ambix.agent import lane, router
from imas_ambix.agent.router import RouterApp, Upstream


class _RefusedMetricsResolver:
    async def resolve(self):
        return (Upstream("http://127.0.0.1:1"),)


class _MetricsResolver:
    def __init__(self, base_url: str) -> None:
        self._base_url = base_url

    async def resolve(self):
        return (Upstream(self._base_url),)


_SGLANG_METRICS = '''\
sglang:max_total_num_tokens{model_name="fixture",tp_rank="0"} 4000000
sglang:num_running_reqs{model_name="fixture",tp_rank="0"} 2
sglang:num_queue_reqs{model_name="fixture",tp_rank="0"} 0
sglang:full_token_usage{model_name="fixture",tp_rank="0"} 0.25
'''


def test_lane_publish_refusal_names_target_and_exception(tmp_path, monkeypatch, caplog):
    """A refused metrics connection must not make an unavailable lane silent."""

    async def exercise() -> None:
        published = asyncio.Event()
        writer = lane.write_unavailable_document

        def record_unavailable(reason, destination):
            result = writer(reason, destination)
            published.set()
            return result

        monkeypatch.setattr(lane, "write_unavailable_document", record_unavailable)
        app = RouterApp(
            _RefusedMetricsResolver(),
            lane_document=tmp_path / "lane.json",
            lane_interval=3600,
        )
        task = asyncio.create_task(app._publish_lane())
        try:
            await asyncio.wait_for(published.wait(), timeout=2)
        finally:
            task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await task
            if app._session is not None:
                await app._session.close()

        lane_state = json.loads((tmp_path / "lane.json").read_text(encoding="utf-8"))
        assert lane_state["state"] == "unavailable"

    with caplog.at_level(logging.WARNING, logger=router.__name__):
        asyncio.run(exercise())

    messages = [record.getMessage() for record in caplog.records]
    assert any("http://127.0.0.1:1/metrics" in message for message in messages)
    assert any("URLError" in message for message in messages)
    assert any("Connection refused" in message for message in messages)


def test_lane_publish_reads_gzip_metrics_body(tmp_path, monkeypatch):
    """The standing publisher must parse a compressed metrics response."""

    async def exercise() -> None:
        async def metrics(_: web.Request) -> web.Response:
            return web.Response(
                body=gzip.compress(_SGLANG_METRICS.encode("utf-8")),
                headers={"Content-Encoding": "gzip"},
            )

        application = web.Application()
        application.router.add_get("/metrics", metrics)
        runner = web.AppRunner(application)
        await runner.setup()
        site = web.TCPSite(runner, "127.0.0.1", 0)
        await site.start()
        sockets = site._server.sockets
        port = sockets[0].getsockname()[1]

        published = asyncio.Event()
        writer = lane.write_lane_document

        def record_document(capacity, destination, **kwargs):
            result = writer(capacity, destination, **kwargs)
            published.set()
            return result

        monkeypatch.setattr(lane, "write_lane_document", record_document)
        app = RouterApp(
            _MetricsResolver(f"http://127.0.0.1:{port}"),
            lane_document=tmp_path / "lane.json",
            lane_interval=3600,
        )
        task = asyncio.create_task(app._publish_lane())
        try:
            await asyncio.wait_for(published.wait(), timeout=2)
        finally:
            task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await task
            if app._session is not None:
                await app._session.close()
            await runner.cleanup()

        lane_state = json.loads((tmp_path / "lane.json").read_text(encoding="utf-8"))
        assert lane_state["state"] == "measured"
        assert lane_state["pool_tokens"] == 4_000_000

    asyncio.run(exercise())


def _sglang_metrics(pool_tokens: int, occupancy: float, running: int) -> str:
    """One SGLang exposition carrying a known pool, occupancy and running count."""
    labels = '{model_name="fixture",tp_rank="0"}'
    return "".join(
        [
            f"sglang:max_total_num_tokens{labels} {pool_tokens}\n",
            f"sglang:num_running_reqs{labels} {running}\n",
            f"sglang:num_queue_reqs{labels} 0\n",
            f"sglang:full_token_usage{labels} {occupancy}\n",
        ]
    )


# Each case names the pool the engine reports, the pool-occupancy fraction it
# reports, and how many requests are running. The mean working context the lane
# derives is round(pool * occupancy / running), and the automatic width is
# floor(pool * 0.90 / that mean), clamped to [16, 36]. The three cases land
# inside the range and on each clamp boundary, so the test shows the arithmetic
# rather than a single point of it: an unclamped 30, a floor-limited 16, and a
# cap-limited 36.
_AUTO_WIDTH_CASES = [
    (4_000_000, 0.30, 10, 30),
    (4_000_000, 0.10, 1, 16),
    (4_000_000, 0.01, 1, 36),
]


@pytest.mark.parametrize(
    ("pool_tokens", "occupancy", "running", "expected"),
    _AUTO_WIDTH_CASES,
)
def test_lane_publish_feeds_the_gates_auto_width(
    tmp_path, pool_tokens, occupancy, running, expected
):
    """The publish path itself must size the gate, not only a direct feed.

    The automatic width is computed from the pool and the working context the
    refresh folds in, so this drives ``_publish_lane`` for one refresh against a
    stub engine metrics endpoint instead of calling the gate's own observation
    entry point. A gate that is only ever seeded directly reads correct in every
    test while the running router silently holds the floor width.
    """
    assert router.DEFAULT_AUTO_OCCUPANCY_TARGET == 0.90
    assert router.DEFAULT_AUTO_WIDTH_FLOOR == 16
    assert router.DEFAULT_AUTO_WIDTH_CAP == 36

    mean_context = round(pool_tokens * occupancy / running)
    raw = math.floor(pool_tokens * 0.90 / mean_context)
    assert expected == max(16, min(raw, 36))

    async def exercise() -> dict:
        async def metrics(_: web.Request) -> web.Response:
            return web.Response(
                text=_sglang_metrics(pool_tokens, occupancy, running),
                content_type="text/plain",
            )

        application = web.Application()
        application.router.add_get("/metrics", metrics)
        runner = web.AppRunner(application)
        await runner.setup()
        site = web.TCPSite(runner, "127.0.0.1", 0)
        await site.start()
        port = site._server.sockets[0].getsockname()[1]

        lane_path = tmp_path / "lane.json"
        # Written where the router derives its own gate file from the lane
        # document, so the wiring under test is the router's, not the test's.
        (tmp_path / router.GATE_FILENAME).write_text(
            json.dumps({"width": "auto"}), encoding="utf-8"
        )
        app = RouterApp(
            _MetricsResolver(f"http://127.0.0.1:{port}"),
            lane_document=lane_path,
            lane_interval=3600,
        )
        task = asyncio.create_task(app._publish_lane())
        loop = asyncio.get_running_loop()
        deadline = loop.time() + 5
        document: dict = {}
        try:
            while loop.time() < deadline:
                try:
                    document = json.loads(lane_path.read_text(encoding="utf-8"))
                except (OSError, json.JSONDecodeError):
                    document = {}
                if "router_generation_gate" in document:
                    break
                await asyncio.sleep(0.02)
        finally:
            task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await task
            if app._session is not None:
                await app._session.close()
            await runner.cleanup()
        assert "router_generation_gate" in document, "no gate snapshot was published"
        return document

    document = asyncio.run(exercise())
    gate_state = document["router_generation_gate"]
    assert document["pool_tokens"] == pool_tokens
    assert gate_state["enabled"] is True
    assert gate_state["effective_width"] == expected
    # The width is only this number because the refresh folded the engine's own
    # reading in: a gate that never saw one holds the floor, so the estimate is
    # what separates a sized width from a width that merely happens to match.
    assert gate_state["context_estimate"] == pytest.approx(mean_context)
